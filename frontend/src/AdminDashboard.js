...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    axios
      .get(`${API_BASE_URL}/admin/drivers`)
      .then((res) => setDrivers(res.data))
      .catch((err) => console.error("기사 목록 불러오기 실패:", err));

    axios
      .get(`${API_BASE_URL}/admin/deliveries`) // ✅ 백엔드 API 경로
  ...
  const addDriver = async () => {
    ...
    try {
      await axios.post(`${API_BASE_URL}/admin/drivers`, {
        name,
        phone,
        password,
        region,
  ...
  const removeDriver = async (index) => {
    ...
    if (target?.id) {
      await axios.delete(`${API_BASE_URL}/admin/drivers/${target.id}`);
      const res = await axios.get(`${API_BASE_URL}/admin/drivers`);
      setDrivers(res.data);
    }
  };
  ...
  const updateDeliveryStatus = async (id, newStatus) => {
    try {
      await axios.patch(`${API_BASE_URL}/deliveries/${id}`, {
        driver_status: newStatus,
      });
      const res = await axios.get(`${API_BASE_URL}/admin/deliveries`);
